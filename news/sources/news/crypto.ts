...
class CryptoAsSource{
  private target: string = 'https://financialmodelingprep.com/api/v4/crypto_news';
  private apikey: string = env.FMP_API_KEY;
  constructor(public pageSize: number= 5, public maxPageNumber: number= 1){}

  /**
   * Asynchronously builds the news articles for the Crypto source.
   *
  ...
  async build(): Promise<void>{
    ...
    try {
      ...
      log("error", "News > Cypto:: Error inserting articles to MongoDB or producing messages", error);
    }
  }
}
  
export default CryptoAsSource;