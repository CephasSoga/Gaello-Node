...
import log from "../../../utils/logging";
  ...
  class FmpAsSource{
    private target: string = 'https://financialmodelingprep.com/api/v3/fmp/articles';
    private apikey: string = env.FMP_API_KEY;
    constructor(public pageSize: number= 5, public maxPageNumber: number= 1){}

  /**
   * Asynchronously builds the news articles for the FMP source.
   *
   * This function fetches paginated data from the FMP API, processes the articles,
   ...
   */
    async build(): Promise<void>{
      ...
      try {
        ...
        log("error", "News > FMP:: Error inserting articles to MongoDB or producing messages", error);
      }
    }
  }
  
export default FmpAsSource;  
